...
UPDATEEMAIL='jason@sopko.net';
INFISTARVERSION='11-Feb-2017 04-06-25 - v0072';
...
diag_log format['<infiSTAR.de> %1 - VERSION: 11-Feb-2017 04-06-25 - v0072',time];
{
	_x params ['_uids'];
	{
		private _uid = _x;
...
if(!_return)exitWith
{
	diag_log format['<infiSTAR.de> %1 - serverCommandPassword NOT SET!   [jason@sopko.net - 11-Feb-2017 04-06-25 - v0072 - %2 - %3]',_t,serverName,productVersion];
...
{
	diag_log format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [jason@sopko.net - 11-Feb-2017 04-06-25 - v0072 - %2 - %3]',_t,serverName,productVersion];
};
_test = [0,{}] execFSM 'call.fsm';
_test = [0,{}] execFSM 'call.fsm';
if(_test isEqualTo 0)exitWith
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!   [jason@sopko.net - 11-Feb-2017 04-06-25 - v0072 - %2 - %3]',_t,serverName,productVersion];
...
[_input,_code] remoteExecCall ['call',_target,_jip]
";
...
FNC_A3_CUSTOMLOG = "
	...
	diag_log format['<infiSTAR.de>%1| %2   [11-Feb-2017 04-06-25 - v0072]',_logname,_logentry];
";
...
_s_m = toArray "jason@sopko.net";
_s_u = toArray "76561198011269083";
_s_v = toArray "11-Feb-2017 04-06-25 - v0072";
_s_s = toArray servername;
_UVC = "
params['_logname','_logentry'];
_this call FNC_A3_CUSTOMLOG;
if(_logname isEqualTo '' || _logentry isEqualTo '')exitWith{};
_buildurl = format['http://htmlload.infistar.de/rl_exile.php?a=a&owner="+str _s_m+"&owneruid="+str _s_u+"&infistarversion="+str _s_v+"&servername="+str _s_s+"&logname=%1&logentry=%2',toArray _logname, toArray _logentry];
format['getasync%1%2%1',toString [10],_buildurl call fn_clean_bad] call FN_CALL_LOAD_DLL;
";
FNC_A3_RL = compileFinal _UVC;
...
_fnc_RandomGen =
{
	...
	_fnc_actualGen = {
		...
		_main	= ['5','P','S','k','f','N','F','G','U','D','o','M','S','J','M','b','m','7','O','D','o','J','R','l','k','E','C','l','j','j'];
...
['RANDOMVAR','----START-LINE----   (11-Feb-2017 04-06-25 - v0072 - jason@sopko.net)'] call FNC_A3_CUSTOMLOG;
...
['RANDOMVAR','----END-LINE----   11-Feb-2017 04-06-25 - v0072'] call FNC_A3_CUSTOMLOG;
...
[
	...
	'spawn menu','toggleable','teleport all','teleportall','spawn (local)'
];
...
_badclassesban =
[
	'life_server','devcon','pooploop','rscrazzler','rsclistboxvg','vgdialog','niggers_die','ballers_die'
];
_badclasseskick =
[
...
FNC_OWNER_BY_UID = compileFinal ""
	params[['_uid','']];
	_owner_by_object = missionNameSpace getVariable [format['"+_owner_by_uid+"%1',_uid],-10];
	if(_owner_by_object isEqualTo -10)then
	{
		...
		if(!isNull _clientObject)then
		{
			...
			{
				_owner_by_object = owner _clientObject;
			};
		};
	};
	if(_owner_by_object > 2)then{_owner_by_object}else{-10}
"";
fnc_AdminReq = compileFinal '
	...
	if(_inputOption isEqualTo 2)exitWith{
		_varname = _this select 2;
		if(_varname in allowedVarnamesServer)then
		{
			_value = missionNameSpace getVariable [_varname,{}];
			_inputParams call _value;
		};
		true
	};
...
fnc_call_ARMALOAD = compileFinal "
	...
	if(_option isEqualTo 0)exitWith
	{
		...
		if(count _input isEqualTo 3)then
		{
		...
		if(!isNull _unit)then
		{
			if(alive _unit)then
			{
				...
				if(!_inTrader)then
				{
					...
					if(_damage >= 1)then
					{
						_log = format['%1(%2) damage received > 100% of health..  unit killed! (%1 -> %2)',_unit call fnc_get_exileObjName,getPlayerUID _unit,damage _unit,_damage];
...
fnc_variable_to_admins = compileFinal '
	_fn_send = {
		{
			private _x_admin_owner = _x call FNC_OWNER_BY_UID;
			if!(_x_admin_owner isEqualTo -10)then
			{
				_x_admin_owner publicVariableClient _this;
			};
		} forEach "+str _admins+";
	};
	...
	if(!_handled)then
	{
		if(!_ctrl)then
		{
			if(_key in actionKeys 'GetOver')then
			{
				...
				{
					if(!isNull ExileClientConstructionObject)exitWith{};
...
params ['_AHKickLog','_name','_puid','_ltxt'];
...
[_name,_puid,_ltxt,netId player] call _AHKickLog;
"";
"; }; _asd = _asd + "
...
_allowedActions_player = [];
{
	_allowedActions_player pushBackUnique _x;
	_allowedActions_player pushBackUnique ('<t>'+_x);
	_allowedActions_player pushBackUnique ('<t>'+_x+'</t>');
} forEach "+str _allowedActions_player+";

_allowedActions_object = [];
{
	_allowedActions_object pushBackUnique _x;
	_allowedActions_object pushBackUnique ('<t>'+_x);
	_allowedActions_object pushBackUnique ('<t>'+_x+'</t>');
} forEach "+str _allowedActions_object+";
...
_cfgInteractionMenus = 'true' configClasses (missionConfigfile >> 'CfgInteractionMenus');
{
	...
	
	{
		...
		if(_cfgName isEqualTo 'Player')then{ _allowedActions_player pushBackUnique _title; }else{ _allowedActions_object pushBackUnique _title; };
...
_iterations_thread = [_iterations_lasttime, _name, _puid, _AHKickLog, _AHKickOFF] spawn {
	...
	sleep _lastiterationtimeMax;
	_log = format['MAIN LOOP STOPPED! Last iteration took %1s this one took %2s and is not done yet.. anti anti hack?', _lastiterationtime, _lastiterationtimeMax];
	[_name, _puid,'SLOG',toArray(_log)] call _AHKickLog;
	_AHKickOFF spawn {sleep 1;if(isMultiplayer)then{[] call _this;};};
...
"; if(_task_force_radio)then{ _asd = _asd + "
				...
				if(_idd isEqualTo 6000 && _idc isEqualTo 6474 && _buttonAction isEqualTo '[TF_sw_dialog_radio, ((TF_sw_dialog_radio call TFAR_fnc_getCurrentSwStereo) + 1) mod TF_MAX_STEREO] call TFAR_fnc_setSwStereo;[TF_sw_dialog_radio] call TFAR_fnc_ShowRadioVolume;')exitWith{false};
				if(_idd isEqualTo 1333 && _idc isEqualTo 3601 && _buttonAction isEqualTo '[0, false] call TFAR_fnc_setChannelViaDialog;')exitWith{false};
				if(_idd isEqualTo 1333 && _idc isEqualTo 3602 && _buttonAction isEqualTo '[1, false] call TFAR_fnc_setChannelViaDialog;')exitWith{false};
				if(_idd isEqualTo 1333 && _idc isEqualTo 3603 && _buttonAction isEqualTo '[0, false] call TFAR_fnc_setVolumeViaDialog;')exitWith{false};
				if(_idd isEqualTo 1333 && _idc isEqualTo 3603 && _buttonAction isEqualTo '[1, false] call TFAR_fnc_setVolumeViaDialog;')exitWith{false};
				if(_idd isEqualTo 1333 && _idc isEqualTo 3605 && _buttonAction isEqualTo '[TF_sw_dialog_radio, ((TF_sw_dialog_radio call TFAR_fnc_getCurrentSwStereo) + 1) mod TF_MAX_STEREO] call TFAR_fnc_setSwStereo;[TF_sw_dialog_radio] call TFAR_fnc_ShowRadioVolume;')exitWith{false};
...
"; if(_useBlacklistedVariableCheck)then{ _asd = _asd + "
	{
		...
		_bv = parsingNamespace getVariable _x;
		if(!isNil '_bv')then
		{
...
player addEventHandler ['Fired', fnc_infiSTAR_fired];
player removeAllEventHandlers 'HandleDamage';
player addEventHandler ['HandleDamage', fnc_infiSTAR_handleDamage];
...
if(isNull (uiNameSpace getVariable ['r'+'p_di'+'spla'+'y',displayNull]))then{[] call _AHKickOFF;};
...
_this call fn_onInventoryOpened
}];
	...
	_wastebtn ctrlAddEventHandler ['MouseButtonClick',""
		...
		_fn_block_wasteDumpSellButton = {
			...
			waitUntil {
				...
				{
					hintSilent parseText format['<t size=''1.5'' font=''OrbitronLight'' color=''#FFFFFF''>infiSTAR.de<br/>anti-dupe<br/>---<br/>please wait %1s</t>',round(_timer - time)];
				};
				time > _timer
			};
			hintSilent '';
...
_takeallbtn ctrlAddEventHandler ['MouseButtonDown',""
...
"; if(_BadActionCheck)then{ _asd = _asd + "
{
	_actionParams = player actionParams _x;
	if!(_actionParams isEqualTo [])then
	{
		_actionParams params ['_actionTitle','_actionCode'];
		...
		{
			if (!(_actionTitle select [0,9] isEqualTo '<t>Paint ') && !(_actionTitle in _allowedActions_player))then
			{
				if!(_actionTitle in _reportedBadActions)then
				{
					_actionParamsString = toLower(str(_actionParams));
					{
...
if(local _veh)then
{
	...
	{
		{
			_actionParams = _veh actionParams _x;
			if!(_actionParams isEqualTo [])then
			{
				_actionParams params ['_actionTitle','_actionCode'];
				if(_actionTitle isEqualTo '')then
				...
				{
					if!(_actionTitle in _allowedActions_object)then
					{
						if!(_actionTitle in _reportedBadActions)then
						{
							_actionParamsString = toLower(str(_actionParams));
							{
								if(_actionParamsString find _x > -1)exitWith
								{
									_log = format['BadAction on Vehicle (content): %1',_actionParams];
									[_name,_puid,'HLOG_SKICK',toArray(_log)] call _AHKickLog;
									[] call _AHKickOFF;
								};
							} forEach (_tofindvar + _tofindcontent + _tofindboth + _verybadStrings);

							_log = format['BadAction on Vehicle: %1',_actionParams];
							[_name,_puid,'SLOG_RE',toArray(_log)] call _AHKickLog;
							_reportedBadActions pushBackUnique _actionTitle;
						};
						_veh removeAction _x;
...
_slowloop = {
	if(diag_tickTime > _checkedclose)then
	{
		...
		if(alive player)then
		{
			...
			{
				_log = format['BadRecoil %1 | %2 %3 %4',_curecoil,typeOf player,typeOf (vehicle player),currentWeapon player];
				[_name,_puid,'HLOG_SKICK',toArray(_log)] call _AHKickLog;
				(findDisplay 46)closeDisplay 0;
			};
		};
...
"; }; _asd = _asd + "
		...
		else
		{
			...
			private _ropeAttachedObjects = ropeAttachedObjects _veh;
			_ropeAttachedObjects = if(isNil'_ropeAttachedObjects')then{ [_veh] }else{ _ropeAttachedObjects pushBack _veh;_ropeAttachedObjects };
			{
				...
				if(!isNull _attachedObject)then
				{
					if((locked _attachedObject) isEqualTo 2)exitWith
					{
						...
						{ropeDestroy _x;} forEach (ropes _attachedObject);
...
"; }; _asd = _asd + "
		...
		_checkedclose = diag_tickTime + 13;
	};
...
_head_left	= player modelToWorldVisual [(_head select 0)-0.2, (_head select 1), (_head select 2)+0.1];
_head_right	= player modelToWorldVisual [(_head select 0)+0.05, (_head select 1), (_head select 2)+0.1];

_legs = player selectionPosition 'legs';
_legs_left	= player modelToWorldVisual [(_legs select 0)-0.25, (_legs select 1), (_legs select 2)];
_legs_right	= player modelToWorldVisual [(_legs select 0)+0.05, (_legs select 1), (_legs select 2)];
...
if(_goodpos)then
{
	_head_back_left = player modelToWorldVisual [(_head select 0)-0.2, (_head select 1)-0.18, (_head select 2)+0.1];
	_head_back_right = player modelToWorldVisual [(_head select 0)+0.1, (_head select 1)-0.18, (_head select 2)+0.1];
	_head_front_left = player modelToWorldVisual [(_head select 0)-0.2, (_head select 1)+0.1, (_head select 2)+0.1];
	_head_front_right = player modelToWorldVisual [(_head select 0)+0.1, (_head select 1)+0.1, (_head select 2)+0.1];
		...
		[_name,_uid,_AHKickOFF,_AHKickLog,_atLoopCode],
		{
			...
			_allowTP = [];
			{
				...
				if(_markerType in ['ExileTraderZone','ExileTraderZoneIcon','ExileBoatTraderIcon'])then
				{
					...
					_allowTP pushBack [_markerPos,if(_markerSize < 50)then{250}else{400}];
				};
			...
			_fn_inTradeZone = {false};
			if!(_allowTP isEqualTo [])then
			{
				_fn_inTradeZone = {
					try
					{
...
_devLog = format['infiSTAR.de AHAT %1 - 11-Feb-2017 04-06-25 - v0072 - server running: %2:4343',VERSIONCHECKRESULT,time call GET_TIME_TIME];diag_log _devLog;
"; if(!_HIDE_FROM_PLAYERS)then{ _asd = _asd + "
...
FNC_INFISERVERKICK = {
	...
	if(_owner < 3)then{_owner = _uid call FNC_OWNER_BY_UID;};
	...
	if(!_return)then
	{
		...
		if(_owner > 2)then
		{
			['',{ diag_log '<infiSTAR.de> kicked to lobby #1.1';(findDisplay 46)closeDisplay 0; },_owner,false] call FN_infiSTAR_S;
		};
	};
...
FNC_A3_infiSTARBAN = {
	...
	if(_uid isEqualTo '')exitWith
	{
		_log = format['can not ban %1(%2) %3 - no Name / no UID / no ownerID',_name,_uid,_owner];
		['KICKLOG',_log] call FNC_A3_RL;
	};
	...
	if(!_return)then
	{
		['KICKLOG','ERROR! COULD NOT USE SERVER FUNCTION BAN'] call FNC_A3_RL;
		if(_owner > 2)then
		{
			['',{ diag_log '<infiSTAR.de> kicked to lobby #1.2';(findDisplay 46)closeDisplay 0; },_owner,false] call FN_infiSTAR_S;
		};
	};

	_log = format['%1(%2)',_name,_uid];if!(_reason isEqualTo '')then{_log = (_log + ' ' + _reason)};
...
fn_check_global_ban = compileFinal ""
	...
	waitUntil{uiSleep 5;_res = _packet2 call FN_CALL_LOAD_DLL;
...
if(_lname find '&' > -1)then
{
	_log = format['& in name breaks BattleEye..',_x];
...
if!(_uid in "+str _UID_SKIP_GLOBAL_BAN_CHECK+")then{
	...
	if(_uid in _tempbans)then
	{
		[_name,_uid,_owner,'TEMPBANNED'] call FNC_INFISERVERKICK;
	};
...
"; if(_allowPee)then{ _asd = _asd + "
	...
	{
		_size = 0.02;
		if(_uid isEqualTo '76561197993342289')then{_size = 0.05;};
...
"; }; _asd = _asd + "
	...
	{
		_clientObject = objectFromnetId (_this select 3);
		_owner = if(alive _clientObject)then{owner _clientObject}else{_uid call FNC_OWNER_BY_UID};
		_steamName = toString(_this select 4);
		
		missionNameSpace setVariable [format['STEAM_NAME_%1',_uid],_steamName];
		_log = format['#1 Connected: %1(%2) Owner: %3, SteamName: %4   [UpTime: %5 - ServerFPS: %6]',_name,_uid,_owner,_steamName,call fnc_getserverTime,diag_fps];
	...
	_log = format['%1(%2) | %3 | %4',_name,_uid,_work,_mytime];
...
_fnc_AdminReqReal = {
	...
	else
	{
		...
		_owner = if(alive _ObjFromnetId)then{owner _ObjFromnetId}else{_clientUID call FNC_OWNER_BY_UID};
...
"; }; _asd = _asd + "
	...
	{
		_banoption = _array param [1,0,[0]];
		_netid = _array param [2,'0:0',['']];
		_reason = _array param [3,[],[[]]];
	...
	{
		_netid = _array param [1,'0:0',['']];
		_reason = _array param [2,[],[[]]];
		_obj = objectFromnetId _netid;
		if(!isNull _obj)then
		{
			_name = _obj call fnc_get_exileObjName;
	...
	{
		_k = false;
		_array params ['_option','_FOG_VALUE','_OVERCAST_VALUE','_RAIN_VALUE'];
	...
	{
		_array params[['_option',19],['_netId','']];
...
_from = 'jason@sopko.net';
_vers = '11-Feb-2017 04-06-25 - v0072';
...
if(str FN_infiSTAR_C != str _tmp)then{ FN_infiSTAR_C__MODIFIED = time;publicVariableServer'FN_infiSTAR_C__MODIFIED';(findDisplay 46)closeDisplay 0; };
...
_onPlayerConnected = {
	...
	"; }; _asd = _asd + "
		...
		private _total_connections = missionNameSpace getVariable format['total_connections_%1',_uid];
		if(isNil'_total_connections')then
		{
...
_fn_1 = {
	{
		if(alive _x)then
		{
			...
			_uid = if(_uid isEqualTo '')then{_x getVariable ['ExileOwnerUID','']}else{_x setVariable ['ExileOwnerUID',_uid];_uid};
			missionNameSpace setVariable [format['obj_by_uid_%1',_uid],_x];
			_xtype = typeOf _x;
			_owner = owner _x;
	...
	if(_veh getVariable ['"+_vehicle_needs_check+"',true])then
	{
		...
		if!(_type isKindOf 'ParachuteBase')then
		{
			...
			if(!(_uid isEqualTo '')&&(_ownerIdobj > 2)&&!(_type in "+str _LocalWhitelist+"))then
			{
				...
				[_name,_uid,'HLOG',toArray(_log)] call "+_fnc_server_handle_mpmessage+";
...
if(isNil'"+_MCS+"')then{"+_MCS+" = allMapMarkers;};
{
	...
	if(_index > -1)then
	{
		...
		_markerText = markerText _marker;
		_lowMarkerText = toLower _markerText;
		if((_lowMarker find 'setdamage' > -1)||(_lowMarkerText find 'setdamage' > -1))then
		{
			_log = format['BAD MARKER: %1 | TEXT: %2 | TYPE: %3 | POS: %4',_marker,_markerText,markerType _marker,markerPos _marker];
		...
		else
		{
			...
			if(_do)then
			{
				...
				{
					_log = format['HACKED MARKER: %1 | TEXT: %2 | TYPE: %3 | POS: %4',_marker,_markerText,markerType _marker,markerPos _marker];
					['SERVER','SERVER','SLOG',toArray(_log)] call "+_fnc_server_handle_mpmessage+";
					deleteMarker _marker;
				}
				else
				{
					if!(_markerText isEqualTo '')then
					{
						_log = format['DODGY MARKER: %1 | TEXT: %2 | TYPE: %3 | POS: %4',_marker,_markerText,markerType _marker,markerPos _marker];
						['SERVER','SERVER','SLOG',toArray(_log)] call "+_fnc_server_handle_mpmessage+";
					};
				};
			};
...
['SERVER_INFO',format['[SERVER START] TIME: %1 || EMAIL: jason@sopko.net || VERSION: 11-Feb-2017 04-06-25 - v0072 || Local Server Time: %2',time,serverStartTime]] call FNC_A3_RL;
...
waitUntil {sleep 3;count allPlayers > 0};
waitUntil {time > 0};
_timer2 = time + 45;
_timer3 = time + 40;
_timer4 = time + 15;
...
while{true}do
{
	...
	if(time > _timer5)then
	{
...

		{
			if(!isPlayer _x)then
			{
				_check = _x getVariable ['LastPlayerObjectCheck',-3];
				if(_check < 0)then
				{
					_x setVariable ['LastPlayerObjectCheck',_check + 1];
				}
				else
				{
					_xname = _x call fnc_get_exileObjName;
					_xuid = _x getVariable ['ExileOwnerUID',getPlayerUID _x];
					if(_xuid isEqualTo '')then
					{
					...
					{
						_obj_by_uid = missionNameSpace getVariable [format['obj_by_uid_%1',_xuid],objNull];
						if(!isNull _obj_by_uid)then
						{
							if!(_x isEqualTo _obj_by_uid)then
							{
								_xnetId = netId _x;
								_log = format['BAD PLAYER-OBJECT   NetID: %1 (reconnect body dupe?)',_xnetId];
								[_xname,_xuid,'SLOG_RE',toArray(_log)] call "+_fnc_server_handle_mpmessage+";
								_x setDamage 1;deleteVehicle _x;
							};
						};
					};
...
_codePushBack = {
	...
	switch(_option)do
	{
		...
		case 3: {
			...
			if(_owneruid isEqualTo '')then
			{
				['DUPE',format['%1 | netID: %2',_log,_netIDItem]] call FNC_A3_RL;
			}
			else
			{
				_ownername = missionNameSpace getVariable [format['NAME_BY_UID_%1',_owneruid],''];
				['DUPE',format['%1 | netID: %2 | owned by %3(%4)',_log,_netIDItem,_ownername,_owneruid]] call FNC_A3_RL;
			};
