...
VERSION_DATE_IS = '11-Feb-2017 04-06-25';
...
fn_STAR_validclass = { (getText(_x >> 'picture') call fn_STAR_validpic) && {(configName _x) find '_Abstract' isEqualTo -1} && {toLower(configName _x) find '_base' isEqualTo -1} };
...
fnc_initOnKeyPress = {
	...
	_ctrl ctrlSetText format['Players loaded in: %1 of %2                 SERVER UPTIME: %3  SERVER FPS: %4  SERVER THREADS: %5          infiSTAR.de Admin Menu 11-Feb-2017 04-06-25 - v0072',count allPlayers,((playersNumber west)+(playersNumber east)+(playersNumber civilian)+(playersNumber resistance)),time call GET_TIME_TIME,SERVER_FPS,SERVER_THREADS];
...
fnc_LBDblClick_RIGHT = {
	...
	_lbData = lbData[RIGHT_SHIFT_ID,(lbCurSel RIGHT_SHIFT_ID)];
	if(_click isEqualTo '')exitWith{};
	if(_lbData in PVAH_AHTMPBAN)exitWith{
		[-667,_lbData] call fnc_AdminReq;
...
FN_CHANGE_VIEWDISTANCE = {
	...
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick','[16,FOG_VALUE,OVERCAST_VALUE,RAIN_VALUE] call fnc_AdminReq;'];
};
...
fnc_infiAdminKeyDown = {
	...
	if(_keycode isEqualTo 0x0E)then
	{
		...
		{
			[1,netId player,prevLoc] call fnc_AdminReq;
			prevLoc = nil;
		};
	};
	_return
...
fnc_infiAdminKeyUp = {
	...
	if(_keycode in (actionKeys 'User4'))then{ if('Arsenal' call ADMINLEVELACCESS)then{ ['Open',true] call BIS_fnc_arsenal; }; };
	false
};
