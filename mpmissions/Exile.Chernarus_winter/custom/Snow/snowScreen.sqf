	...
	_bodyTemp = _this select 3;
        ...
        if (_snowBodyTemp != _bodyTemp || _changeTemp != _tTemp) then {
    	    if(_snowBodyTemp >= 37) then {
      		    _ctrl ctrlsetText "custom\Snow\textures\snowM0.paa";
     		    _ctrl ctrlSetPosition _pos;
     		    _ctrl ctrlcommit 0;	  
		    };
    	    if((_snowBodyTemp <= 36) && (_tTemp < 2)) then {
      		    _ctrl ctrlsetText "custom\Snow\textures\snowM1.paa";
      		    _ctrl ctrlSetPosition _pos;
      		    _ctrl ctrlcommit 0;	  
		    };
    	    if((_snowBodyTemp <= 35) && (_tTemp < 2)) then {
      		    _ctrl ctrlsetText "custom\Snow\textures\snowM2.paa";
      		    _ctrl ctrlSetPosition _pos;
      		    _ctrl ctrlcommit 0;	  
		    };
    	    if((_snowBodyTemp <= 36) && (_tTemp >= 2)) then {
      		    _ctrl ctrlsetText "custom\Snow\textures\snowM4.paa";
      		    _ctrl ctrlSetPosition _pos;
      		    _ctrl ctrlcommit 0;	  
		    };
    	    if((_snowBodyTemp <= 35) && (_tTemp >= 2)) then {
      		    _ctrl ctrlsetText "custom\Snow\textures\snowM3.paa";
      		    _ctrl ctrlSetPosition _pos;
      		    _ctrl ctrlcommit 0;	  
		    };			
	    };
...
while {true} do {
	...
	_selectFog = selectRandom ["custom\Snow\textures\fogM1.paa"];
